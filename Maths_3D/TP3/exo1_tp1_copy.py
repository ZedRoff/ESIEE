...
def main():
    ...
    direction, turn, vertical_movement = check_turn_direction(a, b, c)
    ...
    while not pr.window_should_close():
        ...
        for i in range(len(points)-2):
            cross_product_res, turn_direction_res, vertical_movement_res = check_turn_direction(points[i], points[i+1], points[i+2])
            ...
            draw_text_if_visible_3(camera, turn_direction_res, text_turn_direction_position, 10, pr.BLACK)
            draw_text_if_visible_3(camera, vertical_movement_res, text_vertical_movement_position, 10, pr.BLACK)
    pr.close_window()
