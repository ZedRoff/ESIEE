...
import math
from exo1_tp1_copy import dot_product, vector_length
from exo1 import translation_matrix
...
def rotation_matrix_x(cos_a, sin_a):
    """Creates a 4x4 homogeneous rotation matrix around the X-axis (Roll)."""
...
def rotation_matrix_y(cos_a, sin_a):
    """Creates a 4x4 homogeneous rotation matrix around the Y-axis (Pitch)."""
...
def rotation_matrix_z(cos_a, sin_a):
    """Creates a 4x4 homogeneous rotation matrix around the Z-axis (Yaw)."""
    return np.array([
        [cos_a, -sin_a, 0, 0],
        [sin_a, cos_a, 0, 0],
        [0, 0, 1, 0],
...
def apply_transformation_to_segment(position, rotation_matrix, reference_position, parent_transformation):
    ...
    # TODO : 
    T_to_origin = translation_matrix(-reference_position.x, -reference_position.y, -reference_position.z)

    # Étape 2 : Matrice de translation inverse pour revenir de l'origine
    # TODO : 
    T_from_origin = translation_matrix(reference_position.x, reference_position.y, reference_position.z)
...
def calculate_fingers_positions(wrist, wrist_rotation, FINGER_LENGTH):
    ...
    for i in range(-1, 2):  # Index pour le décalage des doigts
        ...
        # TODO : utilisez la fonction apply_rotation pour faire pivoter les points
...
def draw_human_arm(shoulder, elbow, wrist, finger_positions):
    ...
    for finger_base_global, finger_tip_global in finger_positions:
        pr.draw_sphere(finger_base_global, 0.1, pr.GREEN)  
        pr.draw_cylinder_ex(finger_base_global, finger_tip_global, 0.05, 0.05, 8, pr.YELLOW)  
        phalange_proximale = (finger_base_global.x * 0.7 + finger_tip_global.x * 0.3,
                          finger_base_global.y * 0.7 + finger_tip_global.y * 0.3,
                          finger_base_global.z * 0.7 + finger_tip_global.z  * 0.3)
    
        phalange_mediane = (finger_base_global.x * 0.5 + finger_tip_global.x * 0.5,
                         finger_base_global.y * 0.5 + finger_tip_global.y * 0.5,
                         finger_base_global.z  * 0.5 + finger_tip_global.z  * 0.5)
    
        phalange_distale = (finger_base_global.x * 0.3 + finger_tip_global.x * 0.7,
                         finger_base_global.y * 0.3 + finger_tip_global.y * 0.7,
                         finger_base_global.z  * 0.3 + finger_tip_global.z  * 0.7)
    
  
        pr.draw_sphere(finger_base_global, 0.1, pr.GREEN)  
        pr.draw_sphere(phalange_proximale, 0.08, pr.RED)  
        pr.draw_sphere(phalange_mediane, 0.08, pr.RED)  
        pr.draw_sphere(phalange_distale, 0.08, pr.RED)  
        pr.draw_sphere(finger_tip_global, 0.1, pr.GREEN)  
...
def main():
    ...
    while not pr.window_should_close():  # Boucle principale
        ...
        # TODO : déverrouiller la vérification une fois le code implémenté.
